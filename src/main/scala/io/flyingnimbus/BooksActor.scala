...
final case class Book(title: String, subTitle: String = "", genre: String, description: String, isbn: String, author: String)

final case class Books(books: Seq[Book])
...
object BooksActor {
  ...
  def props(repository: UserRepository): Props = Props(new BooksActor(repository))

}

class BooksActor(repository: UserRepository) extends Actor with ActorLogging {
  ...
  val books = Set(
    Book(
      ...
      genre = "History of Civilisation & Culture",
      isbn = "0099590085"),
    Book(
      title = "Homo Deusw",
  ...
  def receive: Receive = {
    case GetBooks => sender() ! Books(books.toSeq)
  }
}
